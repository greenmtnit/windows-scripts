...
$UpgradeWindows10 = $false # GMITS Setting
...
$RebootAfterUpgrade = $false # GMITS Setting
...
$AttemptUpdateAssistant = $false # GMITS Setting
...
$RemoveRegistryBlocks = $true # GMITS Setting # Removes registry settings so future upgrades aren't blocked
...
# GMITS Customization - Log File
$logDir = "C:\!TECH\WindowsUpgradeLogs"
$script:LogFile = "$logDir\WindowsUpgrade_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
...
function Write-Log { # GMITS Customization
    param(
        [string]$Message,
...
function Exit-WithError {
    param ($Text)
    if ($Datto) {
        Write-Log "<-Start Result->Alert=$Text<-End Result->"
    } elseif ($Syncro) {
        Write-Log $Text
...
function Invoke-Reboot {
    if ($RebootAfterUpgrade) {
        Write-Log "Reboot variable enabled, rebooting in $RebootDelay minutes."
        ...
        Write-Log -Level WARN -Message "Reboot is needed, but reboot variable is not set. Please reboot manually."
    }
}
...
function Get-Download {
    ...
    Write-Log "Downloading: $URL ($([math]::round($DownloadSize/1MB, 2)) MB)`nDestination: $TargetFolder\$FileName..."
    ...
    } else { Write-Log 'File with same size already exists at download target.' }
}
...
function Invoke-CumulativeUpdate {
    ...
    $BeginTime = Get-Date
    Write-Log "Waiting for Cumulative Update to install..."
    ...
    if (((New-Object -ComObject 'Microsoft.Update.Session').QueryHistory("", 0, 1) | Where-Object { $_.Date -gt $BeginTime -and $_.Title -like "*$KB*" }).ResultCode -eq "4") {
        ...
        if ($minutes -eq $TimeToWaitForCompletion) {
            ...
            Write-Log "Cumulative Update has completed successfully."
            Clear-Alert
        }
        Invoke-Reboot
    }
...
function Invoke-EnablementUpgrade {
    ...
    try {
        Write-Log "Adding the enablement package to the image..."
        # GMITS Customization - Fixed argument list and DISM call
        $Arguments = @("/Online", "/Add-Package", "/PackagePath:`"$TargetFolder\$CABFile`"", "/Quiet", "/NoRestart")
        $Process = Start-Process 'dism.exe' -ArgumentList ($Arguments -join " ") -Wait -NoNewWindow
        if ($Process.ExitCode -eq '3010') {
            Write-Log "Exit Code 3010: Package added successfully."
            Clear-Alert
        ...
        Write-Log "Enablement package install failed. Error: $($_.exception.Message)"
        if ($AttemptUpdateAssistant) {
            Write-Log "Attempting Update Assistant method instead."
...
function Invoke-UpdateAssistant {
    ...
    if ($DiskSpace -lt $DiskSpaceRequired) {
        ...
        if (-not (Test-Path "$TargetFolder\$UAFile")) {
            ...
            Write-Log "Update Assistant download successful, starting upgrade."
            if ((Get-Process 'Windows10UpgraderApp' -ErrorAction SilentlyContinue).Count -gt 0) {
                ...
                Write-Log "Waiting for Update Assistant to complete..."
                $BeginTime = Get-Date
                ...
                if ($minutes -eq $TimeToWaitForCompletion) {
                    ...
                    Write-Log "Update Assistant completed successfully."
                    Clear-Alert
                }
                Invoke-Reboot
            }
...
# GMITS Customization - Initialize Logging
New-Item -ItemType Directory $logDir -ErrorAction SilentlyContinue
New-Item $script:LogFile
Write-Log -Message "START" -Level INFO
...
Write-Log "Windows $MajorVersion $DisplayVersion build $Build.$UBR detected."
...
if ($RemoveRegistryBlocks -eq $true -and ((Get-Item $WUPolicyRegLocation -ErrorAction SilentlyContinue).Property -like "TargetReleaseVersion*")) {
    ...
    Write-Log "ProductVersion/TargetRelease registry settings were found and removed."
}

# Ineligible Upgrade Conditions
if ($CurrentVersion.InstallationType -ne 'Client') {
    Write-Log "Device is not running a client OS."
    Start-Sleep 10; exit 0
} elseif ($MajorVersion -lt '11' -and $UpgradeWindows10 -eq $false) {
    Write-Log "Upgrading Windows 10 is disabled in script variables, exiting."
    Start-Sleep 10; exit 0
} elseif ($DisplayVersionNumerical -ge $WinLatestVersionNumerical) {
    Write-Log "Already running $DisplayVersion which is the same or newer than target release $WinLatestVersion, no update required."
    Clear-Alert
    Start-Sleep 10; exit 0
...
if ($AttemptCumulativeUpdate -eq $true -and
    ...
    Write-Log "Windows $MajorVersion UBR's older than $EPRequiredUBR cannot be upgraded with enablement package. Installing required Cumulative Update."
    ...
    Write-Log "Build is not compatible with enablement upgrade, attempting Update Assistant method instead."
    Invoke-UpdateAssistant
} elseif ($MajorVersion -lt '11' -and $UpgradeWindows10 -eq $true -and $AttemptUpdateAssistant -eq $true) {
    Write-Log "Upgrading Windows 10 using Update Assistant method."
    Invoke-UpdateAssistant
} else {
